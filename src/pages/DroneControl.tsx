...
import { MapPin, Search, Battery, Camera, RotateCw, ChevronDown } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
const DroneControl = () => {
  ...
  const DroneDetails = ({ droneId }: { droneId: number }) => (
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="outline" className="w-full mb-4">
            Select Drone <ChevronDown className="ml-2 h-4 w-4" />
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent className="w-full">
          <DropdownMenuItem onClick={() => setSelectedDrone(1)}>
            Drone 1
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => setSelectedDrone(2)}>
            Drone 2
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>

      {selectedDrone && <DroneDetails droneId={selectedDrone} />}
    </div>
  );
};
